...
std::tuple<int, int, int, double> select_turple_from_Lr(const std::vector<std::tuple<int, int, int, double>>& Lr, double total_q, Data& data) {
...
void ProbabilisticInsertion(Solution &s, std::vector<std::vector<int>>& adjMatrix, Data &data){
    ...
    {   
        ...
        {
            // std::unordered_set<int> unrouted_nodes;
            // for (int i = 0; i < index; i++) {
            //     unrouted_nodes.insert(std::get<0>(unrouted[i]));
            // }
            ...
            while (selected == -1 && ptr < (int)target_list.size()) {
                ...
                // if (unrouted_nodes.find(candidate_node) != unrouted_nodes.end()) {
                //     selected = std::distance(unrouted.begin(), std::find_if(unrouted.begin(), unrouted.begin() + index,
                //         [candidate_node](const std::tuple<int, int>& tuple) { return std::get<0>(tuple) == candidate_node; }));
                //     first_node = candidate_node;
                // }
                ptr++;
            }
        ...
        while (index > 0)   //# unrouted customers > 0
        {
            ...
            std::tuple<int, int, int, double> selected_tuple = select_turple_from_Lr(cost_list_r, total_q, data);
